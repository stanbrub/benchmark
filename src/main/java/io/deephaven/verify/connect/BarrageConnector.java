...
import io.deephaven.verify.util.MetricsFuture;
...
public class BarrageConnector implements AutoCloseable {
	...
	public void close() {
		try {
			...
			try { session.session().closeFuture().get(5, TimeUnit.SECONDS); } 
			catch(Exception ex) {}
	...
	private void checkClosed() {
		if(isClosed.get()) throw new RuntimeException("Session is closed");
	}
	
	private FieldInfo findTable(String table) {
		Optional<FieldInfo> found = changes.changes().created().stream().filter(f->f.name().equals(table)).findFirst();
		if(!found.isPresent()) found = changes.changes().updated().stream().filter(f->f.name().equals(table)).findFirst();
		return found.get();
	}
	...
	private ManagedChannel getManagedChannel(String host, int port) {
		...
		if("localhost:10000".equals(host + ':' + port)) channelBuilder.usePlaintext();
		else channelBuilder.useTransportSecurity();
