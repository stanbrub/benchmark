...
/**
 * Represents properties for a the Benchmark API .profile file in addition to allowing retrieval of 
...
class Profile {
	...
	Duration propertyAsDuration(String name, String defaultValue) {
		String value = property(name, defaultValue);
		String timeStr = "([0-9]+)(.*)";
		if(!value.matches(timeStr)) throw new RuntimeException("Bad duration value: " + name + "=" + value);
		...
		switch(split[1].trim()) {
		case "nanos", "nano": return Duration.ofNanos(amount);
		case "millis", "milli": return Duration.ofMillis(amount);
		case "seconds", "second": return Duration.ofSeconds(amount);
		case "minutes", "minute": return Duration.ofMinutes(amount);
		default: throw new RuntimeException("Bad duration time type: " + name + "=" + value);
		}
	}
	...
	private URL findProfileUrl(String uri) {
		...
		if(url == null) url = findProfileAsAbsolutePath(uri);
		if(url == null) url = findProfileInCurrentDir(uri);	
		if(url == null) url = findProfileAsResource(BenchmarkMain.class, "profile/" + uri);
		if(url == null) url = findProfileAsResource(getClass(), uri);
		if(url == null) throw new RuntimeException("Failed to find profile: " + uri);
		return url;
	}
