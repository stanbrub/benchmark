...
public class Classes {
	...
	private void addClasses(String jarPath, Map<String,ClassInfo> classes) {
		try(ZipInputStream in = new ZipInputStream(new BufferedInputStream(Files.newInputStream(Paths.get(jarPath))))) {
			for(ZipEntry e = in.getNextEntry(); e != null; e = in.getNextEntry()) {
				String className = e.getName();
				if(!className.endsWith(".class")) continue;
				ClassInfo info = classes.get(className);
				if(info == null) classes.put(className, info = new ClassInfo(className));
