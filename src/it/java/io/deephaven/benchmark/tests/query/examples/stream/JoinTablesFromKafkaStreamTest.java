...
/**
 * Join two tables that are generated from a Kafka stream. Demonstrates how to configure the 
 * tables, produce data to Kafka topics, consume the topics to Deephaven tables, and 
...
public class JoinTablesFromKafkaStreamTest {
	...
	/**
	 * Join two tables from a Kafka stream generated by the test where one table is fixed size and 
	 * the other is scaled:
	 * <ol>
	 ...
	 * </ol>
	 * This test is identical to {@link #joinTwoTablesFromKafkaStream_Shorthand} except without the 
	 * use of <code>bench_api_</code> functions for Kafka consumers and table waiting.<p/>
	 ...
	 * <li>End the query when all data has been consumed and aggregated</li>
	 * </ol>
	 * This test is identical to {@link #joinTwoTablesFromKafkaStream_Longhand} except without the 
	 * use of <code>bench_api_</code> functions for Kafka consumers and table waiting.<p/>
	 * Properties (e.g. ${kafka.consumer.addr}) are automatically filled in during query execution.
	 */
	@Test 
	...
	/**
	 * Count the records from a Kafka topic as the stream is being produced.  Nothing is 
	 * done with the data other than counting the rows. This shows the overhead of only reading the 
	 * Kafka topic into Deephaven without any other operations:
	 * <ol>
