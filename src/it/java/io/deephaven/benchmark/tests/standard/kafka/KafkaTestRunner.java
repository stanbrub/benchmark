...
class KafkaTestRunner {
    ...
    /**
     * Shut down the Kafka consumer and derived tables in the current session after the test completes.
     * This must run in the same session that created the consumer, since a new session cannot access
     * the previous session's variables. Without this, the old consumer (e.g. JSON) stays alive and
     * reads data generated by the next test (e.g. Protobuf), causing deserialization errors.
     * <p/>
     * Simply calling {@code j_table.close()} is not enough because the underlying {@code KafkaIngester}
     * runs its own consumer loop thread. Dropping the table from the liveness scope triggers the
     * ingester's shutdown and stops the polling thread.
     */
    private void shutdownConsumer() {
        ...
        if 'result' in dir() and result is not None:
            try:
                LivenessScopeStack.peek().release(result.j_table)
            except Exception:
                pass
        result = None

        if 'consumer_tbl' in dir() and consumer_tbl is not None:
            try:
                LivenessScopeStack.peek().release(consumer_tbl.j_table)
            except Exception:
                pass
        consumer_tbl = None
